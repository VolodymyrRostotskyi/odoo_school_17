...
class HrHospitalVisit(models.Model):
    ...
    @api.depends('patient_id', 'scheduled_visit_datetime')
    def _compute_display_name(self):
        for visit in self:
            name_parts = ["Visit"]
            if visit.patient_id:
                name_parts.append(visit.patient_id.name)
            if visit.scheduled_visit_datetime:
                name_parts.append(
                    visit.scheduled_visit_datetime.strftime('%Y-%m-%d'))
            visit.display_name = " – ".join(name_parts)

    def action_confirm(self):
        for rec in self:
            if rec.status != 'scheduled':
                raise ValidationError(
                    _("Only visits in status 'Scheduled' can be confirmed."))
    ...
    def action_cancel(self):
        for rec in self:
            if rec.status != 'scheduled':
                raise ValidationError(
                    _("Only visits in status 'Scheduled' can be cancelled."))
